...
def get_cluster(basic_dict, router_dict):
    ...
    #router_name---router_position
    #basic_name---basic_position

    for i in range(len(basic_dict)):#遍历每个普通节点
        min_dis = np.inf#初始值 极大值
        index = 0
        for j in range(len(router_dict)):#TODO 优化 .items() 遍历所有簇头节点
